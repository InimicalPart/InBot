...
async function runCommand(message, args, RM) {
	...
	if (!require("../../../config.js").cmdMonthly) {
	...
	message.channel.send(new RM.Discord.MessageEmbed().setDescription("<a:loading:869354366803509299> *Working on it...*")).then(async (m) => {
		...
		await connect.updateCooldown("cooldown", message.author.id, undefined, undefined, undefined, undefined, undefined, undefined, new Date(new Date().setTime(new Date().getTime() + (720 * 60 * 60 * 1000))))
		...
		m.edit(new RM.Discord.MessageEmbed()
			...
			.setAuthor(message.author.tag, message.author.avatarURL())
			.setDescription(
				"**Success:** You have been given $100,000!"
			)
			.setThumbnail(message.guild.iconURL())
			.setTitle("Success")
		)
