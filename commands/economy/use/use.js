	...
	message.channel.send(new RM.Discord.MessageEmbed().setDescription("<a:loading:869354366803509299> *Working on it...*")).then(async (m) => {
		if (!args[0]) {
			...
			return m.edit(new RM.Discord.MessageEmbed()
				...
				.setAuthor(message.author.tag, message.author.avatarURL())
				.setDescription(
					"You need to specify an item to use."
				)
				.setThumbnail(message.guild.iconURL())
		...
		if (args[0] === "banknote") {
			...
			if (inventory.banknote == undefined || inventory.banknote < parseInt(amount)) {
				connect.end(true)
				return m.edit(new RM.Discord.MessageEmbed()
					.setColor("RED")
					.setAuthor(message.author.tag, message.author.avatarURL())
			...
			await connect.update("currency", message.author.id, undefined, undefined, maxBank + (1000 * parseInt(amount)))
			...
			return m.edit(new RM.Discord.MessageEmbed()
				...
				.setAuthor(message.author.tag, message.author.avatarURL())
				.setDescription(
					"You have successfully added $" + (1000 * parseInt(amount)) + " to your maximum bank capacity. You max bank capacity is now: `$" + (maxBank + (1000 * parseInt(amount))) + "`"
				)
				.setThumbnail(message.guild.iconURL())
			...
			return m.edit(new RM.Discord.MessageEmbed()
				...
				.setAuthor(message.author.tag, message.author.avatarURL())
				.setDescription(
					"You just ate **" + amount + "** Ray's brain(s), you feel dumber"
				)
				.setThumbnail(message.guild.iconURL())
