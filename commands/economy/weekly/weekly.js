const commandInfo = {
	"primaryName": "weekly", // This is the command name used by help.js (gets uppercased).
	"possibleTriggers": ["weekly"], // These are all commands that will trigger this command.
	"help": "Claim your weekly gift!", // This is the general description of the command.
...
async function runCommand(message, args, RM) {
	...
	if (!require("../../../config.js").cmdWeekly) {
	...
	message.channel.send(new RM.Discord.MessageEmbed().setDescription("<a:loading:869354366803509299> *Working on it...*")).then(async (m) => {
		...
		const weeklyCool = data.weeklycool
		if (weeklyCool !== null) {
			const cooldown = new Date(weeklyCool * 1000)
			const now = new Date()
		...
		await connect.updateCooldown("cooldown", message.author.id, undefined, undefined, undefined, undefined, undefined, new Date(new Date().setTime(new Date().getTime() + (168 * 60 * 60 * 1000))))
		let data2 = await connect.fetch("currency", message.author.id)
		const authorBal = data2.amountw
		await connect.update("currency", message.author.id, authorBal + 30000)
		m.edit(new RM.Discord.MessageEmbed()
			.setColor("GREEN")
			.setAuthor(message.author.tag, message.author.avatarURL())
			.setDescription(
				"**Success:** You have been given $30,000!"
			)
			.setThumbnail(message.guild.iconURL())
			.setTitle("Success")
		)
