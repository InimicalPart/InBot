...
async function runCommand(message, args, RM) {
  if (!require("../../../config.js").cmdVcsounds) {
...
>>>>>>> 82fbc2ea8c9c1ab43c06c7c72143bfbab5fcfbb4
  ...
  if (serverQueue)
    return message.channel.send({
      content: "Cannot Play Music Trivia While Music is Playing!",
    });
  const triviaData = {
  ...
  if (!channel) return message.channel.send({ embeds: [channeljoin] });
  if (serverQueue && serverQueue.playing)
    return message.channel.send({
      content: "**A quiz or a song is already running!**",
    });
  if (message.member.voice.channel.members.size < 2)
    return message.channel.send({
      content: "You can't make a trivia with only yourself! That's no fun :c",
    });
  triviaData.isTriviaRunning = true;

  if (!args[0])
    return message.channel.send({
      content: "**Provide is the number of songs you want the quiz to have?**",
    });
  ...
  if (args[0] >= 5 && args[0] <= 15) {
    ...
    return message.channel.send({
      content: "**Please enter a number between 5 and 15**",
    });
  }
...
>>>>>>> 82fbc2ea8c9c1ab43c06c7c72143bfbab5fcfbb4
          ...
          collector.on("end", function () {
            ...
            const sortedScoreMap = new Map(
              [...triviaData.triviaScore.entries()].sort((a, b) => b[1] - a[1])
            );
