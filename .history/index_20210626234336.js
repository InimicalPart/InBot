...
client.on('message', async (message) => {
	...
	if (message.content.startsWith(prefix + "test")) {
		if (botOwners.includes(message.author.id)) {
			message.channel.send("alr pinging...").then (async (msg) =>{
				msg.delete()
				message.channel.send(`<:bitelip:857350270513971221> latency is ${msg.createdTimestamp - message.createdTimestamp}ms. and API latency is ${Math.round(client.ws.ping)}ms`);
			})
		} else {
